...
NSString *const YYRequestCacheErrorDomain = @"com.mrj.request.caching";

static dispatch_queue_t YYRequest_cache_writing_queue() {
    ...
    dispatch_once(&onceToken, ^{
        ...
        queue = dispatch_queue_create("com.mrj.YYkrequest.caching", attr);
    });
    
    return queue;
}

@interface YYNacheMetadata : NSObject<NSSecureCoding>
...
@implementation YYNacheMetadata
...
@property (nonatomic, strong) YYNacheMetadata *cacheMetadata;
...
- (void)requestCompletePreprocessor {
    ...
    if (self.writeCacheAsynchronously) {
        dispatch_async(YYRequest_cache_writing_queue(), ^{
...
- (BOOL)loadCacheWithError:(NSError * _Nullable __autoreleasing *)error {
    ...
    if (![self loadCacheData]) {
        if (error) {
            *error = [NSError errorWithDomain:YYRequestCacheErrorDomain code:YYRequestCacheErrorInvalidCacheData userInfo:@{ NSLocalizedDescriptionKey:@"Invalid cache data"}];
        }
        return NO;
    }
...
- (BOOL)validateCacheWithError:(NSError * _Nullable __autoreleasing *)error {
    ...
    if (duration < 0 || duration > [self cacheTimeInSeconds]) {
        if (error) {
            *error = [NSError errorWithDomain:YYRequestCacheErrorDomain code:YYRequestCacheErrorExpired userInfo:@{ NSLocalizedDescriptionKey:@"Cache expired"}];
        }
        return NO;
    }
    ...
    if (cacheVersionFileContent != [self cacheVersion]) {
        if (error) {
            *error = [NSError errorWithDomain:YYRequestCacheErrorDomain code:YYRequestCacheErrorVersionMismatch userInfo:@{ NSLocalizedDescriptionKey:@"Cache version mismatch"}];
        }
        return NO;
    }
    // Sensitive data
    ...
    if (sensitiveDataString || currentSensitiveDataString) {
        ...
        if (sensitiveDataString.length != currentSensitiveDataString.length || ![sensitiveDataString isEqualToString:currentSensitiveDataString]) {
            if (error) {
                *error = [NSError errorWithDomain:YYRequestCacheErrorDomain code:YYRequestCacheErrorSensitiveDataMismatch userInfo:@{ NSLocalizedDescriptionKey:@"Cache sensitive data mismatch"}];
            }
            return NO;
        }
    }
    ...
    if (appVersionString || currentAppVersionString) {
        if (appVersionString.length != currentAppVersionString.length || ![appVersionString isEqualToString:currentAppVersionString]) {
            if (error) {
                *error = [NSError errorWithDomain:YYRequestCacheErrorDomain code:YYRequestCacheErrorAppVersionMismatch userInfo:@{ NSLocalizedDescriptionKey:@"App version mismatch"}];
            }
            return NO;
        }
    }
...
- (BOOL)loadCacheMetadata {
    ...
    if ([fileManager fileExistsAtPath:path isDirectory:nil]) {
        @try {
            ...
            YYLog(@"Load cache metadata failed, reason = %@", exception.reason);
            return NO;
        }
    }
    return NO;
