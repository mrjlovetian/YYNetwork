...
void YYLog(NSString *format, ...) {
#ifdef DEBUG
    if (![YYNetworkConfig sharedConfig].debugLogEnabled) {
        return;
    }
    va_list argptr;
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        YYLog(@"error to set do not backup attribute, error = %@", error);
    }
}
...
@implementation YYBaseRequest (RequestAccessory)

- (void)toggleAccessoriesWillStartCallBack {
    for (id<YYRequestAccessory> accessory in self.requestAccessories) {
...
@implementation YYNhainRequest (RequestAccessory)
