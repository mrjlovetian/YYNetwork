...
- (NSString *)buildRequestUrl:(YYBaseRequest *)request {
    ...
    for (id<YYUrlFilterProtocol> f in filters) {
...
- (AFHTTPRequestSerializer *)requestSerializerForRequest:(YYBaseRequest *)request {
    AFHTTPRequestSerializer *requestSerializer = nil;
    if (request.requestSerializerType == YYRequestSerializerTypeHTTP) {
        requestSerializer = [AFHTTPRequestSerializer serializer];
    } else if (request.requestSerializerType == YYRequestSerializerTypeJSON) {
...
- (NSURLSessionTask *)sessionTaskForRequest:(YYBaseRequest *)request error:(NSError **)error {
...
- (void)addRequest:(YYBaseRequest *)request {
    ...
    if ([request.requestTask respondsToSelector:@selector(priority)]) {
        switch (request.requestPriority) {
            case YYRequestPriorityHigh:
                request.requestTask.priority = 1.0;
                break;
            case YYRequestPriorityLow:
                request.requestTask.priority = 0;
                break;
            case YYRequestPriorityDefault:
                /*!!fall through*/
            default:
    ...
    // Retain request
    YYLog(@"Add request: %@", NSStringFromClass([request class]));
...
- (void)cancelAllRequests {
    ...
    if (allKeys && allKeys.count > 0) {
        ...
        for (NSNumber *key in copiedKeys) {
            Lock();
            YYBaseRequest *request = _requestsRecord[key];
            Unlock();
...
- (BOOL)validateResult:(YYBaseRequest *)request error:(NSError * _Nullable __autoreleasing *)error {
    ...
    if (!result) {
        if (error) {
            *error = [NSError errorWithDomain:YYRequestValidationErrorDomain code:YYRequestValidationErrorInvalidStatusCode userInfo:@{NSLocalizedDescriptionKey:@"Invalid status code"}];
        }
        return result;
    }
...
- (void)handleRequestResult:(NSURLSessionTask *)task responseObject:(id)responseObject error:(NSError *)error {
    Lock();
    YYBaseRequest *request = _requestsRecord[@(task.taskIdentifier)];
    Unlock();
    ...
    YYLog(@"Finished Request: %@", NSStringFromClass([request class]));
    ...
    if ([request.responseObject isKindOfClass:[NSData class]]) {
        ...
        switch (request.responseSerializerType) {
            case YYResponseSerializerTypeHTTP:
                ...
                break;
            case YYResponseSerializerTypeJSON:
                request.responseObject = [self.jsonResponseSerializer responseObjectForResponse:task.response data:request.responseData error:&serializationError];
                request.responseJSONObject = request.responseObject;
                break;
            case YYResponseSerializerTypeXMLParser:
...
- (void)requestDidFailWithRequest:(YYBaseRequest *)request error:(NSError *)error {
    request.error = error;
    YYLog(@"Request %@ failed, status code = %ld, error = %@",
...
- (void)addRequestToRecord:(YYBaseRequest *)request {
    Lock();
    _requestsRecord[@(request.requestTask.taskIdentifier)] = request;
    Unlock();
}

- (void)removeRequestFromRecord:(YYBaseRequest *)request {
    Lock();
    [_requestsRecord removeObjectForKey:@(request.requestTask.taskIdentifier)];
    YYLog(@"Request queue size = %zd", [_requestsRecord count]);
    Unlock();
}
...
- (NSURLSessionDownloadTask *)downloadTaskWithDownloadPath:(NSString *)downloadPath
    ...
    if (canBeResumed) {
        @try {
            ...
            YYLog(@"Resume download failed, reason = %@", exception.reason);
            resumeSucceeded = NO;
        }
    }
    if (!resumeSucceeded) {
...
- (NSString *)incompleteDownloadTempCacheFolder {
    ...
    if (!cacheFolder) {
        ...
        cacheFolder = [cacheDir stringByAppendingPathComponent:YYKNetworkIncompleteDownloadFolderName];
    }
    
    NSError *error = nil;
    if(![fileManager createDirectoryAtPath:cacheFolder withIntermediateDirectories:YES attributes:nil error:&error]) {
        YYLog(@"Failed to create cache directory at %@", cacheFolder);
        cacheFolder = nil;
    }
    return cacheFolder;
}
